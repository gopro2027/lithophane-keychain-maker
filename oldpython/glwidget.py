...
class GLWidget(QOpenGLWidget, QOpenGLFunctions):
    ...
    def updateImage(self, img_scr, rembg):
        self.fileDir = img_scr
        self.lithophane.generateVertex(img_scr, rembg)
        self.initializeGL()
    ...
    def generateSTL(self,file_name):
        ...
        triangle_count = 0
        flipz = 1 #set this to -1 to do a z mirror
        for i in range(0,len(v_data)-1,4):
            ...
            file.write("            vertex {} {} {}\n".format(np.float32(q1[0]),np.float32(q1[1]),flipz*np.float32(q1[2])))
            file.write("            vertex {} {} {}\n".format(np.float32(q2[0]),np.float32(q2[1]),flipz*np.float32(q2[2])))
            file.write("            vertex {} {} {}\n".format(np.float32(q4[0]),np.float32(q4[1]),flipz*np.float32(q4[2])))
            file.write("    end loop\n")
            file.write("endfacet\n") 
            ...
            file.write("    outer loop\n")
            file.write("            vertex {} {} {}\n".format(np.float32(q2[0]),np.float32(q2[1]),flipz*np.float32(q2[2])))
            file.write("            vertex {} {} {}\n".format(np.float32(q3[0]),np.float32(q3[1]),flipz*np.float32(q3[2])))
            file.write("            vertex {} {} {}\n".format(np.float32(q4[0]),np.float32(q4[1]),flipz*np.float32(q4[2])))
            file.write("    end loop\n")
            file.write("endfacet\n") 
            triangle_count+=2
